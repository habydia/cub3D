...
void	init_ray(t_game *game, t_ray *ray, int x, int w)
{
	...
	ray->ray_dir_x = game->player_vector_direction_x + game->camera_plane_x * ray->camera_x;
	ray->ray_dir_y = game->player_vector_direction_y + game->camera_plane_y * ray->camera_x;
...
{
	if (ray->side == 0)
		ray->perp_wall_dist = (ray->map_x - game->player_pos_x + (1 - ray->step_x)
				/ 2) / ray->ray_dir_x;
	else
		ray->perp_wall_dist = (ray->map_y - game->player_pos_y + (1 - ray->step_y)
				/ 2) / ray->ray_dir_y;
}
